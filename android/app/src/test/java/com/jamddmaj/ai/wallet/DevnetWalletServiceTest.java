...
public class DevnetWalletServiceTest {
 ...
 @Test public void accessRecoveryPreservesPendingTransaction()throws Exception{var d=draft();rpc.sendTimeout=true;reject(()->service.submit(d,service.signReviewed(d,seed),()->true));String before=service.status(),oldId=service.wallet().getString("id"),next="abcdef1234567890abcdef1234567890";service.recoverExistingAccess(oldId,next,DevnetSolana.address(seed));service=new DevnetWalletService(context,rpc);assertEquals(next,service.wallet().getString("id"));assertEquals(before,service.status());reject(()->draft());assertEquals(1,rpc.sends);}
 @Test public void differentWalletOrStaleRecoveryCannotReplaceAccess()throws Exception{String oldId=service.wallet().getString("id"),next="abcdef1234567890abcdef1234567890";reject(()->service.recoverExistingAccess(oldId,next,recipient));reject(()->service.recoverExistingAccess(next,oldId,DevnetSolana.address(seed)));assertEquals(oldId,service.wallet().getString("id"));}
 @Test public void interruptedRecoveryLeavesPriorMetadataReadable()throws Exception{String oldId=service.wallet().getString("id");Files.write(new File(folder.getRoot(),"devnet-wallet-public.json.new").toPath(),"partial".getBytes(java.nio.charset.StandardCharsets.UTF_8));service=new DevnetWalletService(context,rpc);assertEquals(oldId,service.wallet().getString("id"));assertEquals(DevnetSolana.address(seed),service.wallet().getString("address"));}
 ...
 @Test public void wrongSeedCannotSign()throws Exception{var d=draft();byte[] wrong=new byte[32];reject(()->service.signReviewed(d,wrong));assertEquals(0,rpc.sends);}
 @Test public void unwritableJournalCannotBroadcast()throws Exception{var d=draft();File path=new File(folder.getRoot(),"devnet-transfer-public.json");assertTrue(path.mkdir());Files.write(new File(path,"keep").toPath(),"test obstruction".getBytes(java.nio.charset.StandardCharsets.UTF_8));reject(()->service.submit(d,service.signReviewed(d,seed),()->true));assertEquals(0,rpc.sends);}
 ...
 @Test public void corruptJournalBlocksInsteadOfOverwriting()throws Exception{Files.write(new File(folder.getRoot(),"devnet-transfer-public.json").toPath(),"{}".getBytes(java.nio.charset.StandardCharsets.UTF_8));try{draft();fail("Corrupt journal accepted");}catch(JSONException expected){}assertEquals(0,rpc.sends);}
}
