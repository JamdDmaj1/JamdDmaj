...
public class DevnetWalletServiceTest {
 ...
 @Test public void unwritableJournalCannotBroadcast()throws Exception{var d=draft();File path=new File(folder.getRoot(),"devnet-transfer-public.json");assertTrue(path.mkdir());Files.writeString(new File(path,"keep").toPath(),"test obstruction");reject(()->service.submit(d,service.signReviewed(d,seed),()->true));assertEquals(0,rpc.sends);}
