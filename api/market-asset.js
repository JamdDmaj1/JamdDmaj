...
async function fetchJson(url) {
  ...
  try {
    const response = await fetch(url, { signal: controller.signal, headers: { "Accept": "application/json", "User-Agent": "JamdDmaj-Asset-Research/1.37.62" } });
...
async function fetchText(url) {
  ...
  const timer = setTimeout(() => controller.abort(), FETCH_TIMEOUT_MS);
  try {
    const response = await fetch(url, { signal: controller.signal, headers: { "Accept": "application/rss+xml, application/xml, text/xml", "User-Agent": "JamdDmaj-Asset-Research/1.37.62" } });
