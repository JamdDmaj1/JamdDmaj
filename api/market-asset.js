...
export default async function handler(request) {
  ...
  if (!symbol || !coinId) return jsonResponse(request, { error: { message: "A valid symbol and coinId are required." } }, 400);

  const cacheKey = `${symbol}:${coinId}:${timeframe}`;
...
async function fetchJson(url) {
  ...
  try {
    const response = await fetch(url, { signal: controller.signal, headers: { "Accept": "application/json", "User-Agent": "JamdDmaj-Asset-Research/1.37.57" } });
...
async function fetchText(url) {
  ...
  const timer = setTimeout(() => controller.abort(), FETCH_TIMEOUT_MS);
  try {
    const response = await fetch(url, { signal: controller.signal, headers: { "Accept": "application/rss+xml, application/xml, text/xml", "User-Agent": "JamdDmaj-Asset-Research/1.37.57" } });
...
function cleanSymbol(value) {
  return String(value || "").toUpperCase().replace(/[^A-Z0-9]/g, "").replace(/USDT$/, "").slice(0, 16);
}
