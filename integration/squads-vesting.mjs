...
const connection={getAccountInfo:async key=>{const a=svm.getAccount(key.toBase58());return a?{data:Buffer.from(a.data),owner:new PublicKey(a.programAddress),lamports:Number(a.lamports),executable:a.executable}:null;}};
...
send([SystemProgram.createAccount({fromPubkey:owner.publicKey,newAccountPubkey:mint.publicKey,lamports:10000000,space:82,programId:token.TOKEN_2022_PROGRAM_ID}),token.createInitializeMint2Instruction(mint.publicKey,9,owner.publicKey,null,token.TOKEN_2022_PROGRAM_ID),token.createAssociatedTokenAccountInstruction(owner.publicKey,source,owner.publicKey,mint.publicKey,token.TOKEN_2022_PROGRAM_ID),token.createMintToInstruction(mint.publicKey,source,owner.publicKey,1000000000000000000n,[],token.TOKEN_2022_PROGRAM_ID)],[owner,mint]);
...
const allocations=JSON.parse(readFileSync(new URL('../security/jamd-allocation-plan.json',import.meta.url))).allocations.filter(a=>BigInt(a.lockedTokens)>0n);
const locks=[];
for(const allocation of allocations){
  ...
  locks.push({allocation,vault,destination,claim,amount,start:now()});
}
...
for(const lock of locks){
  ...
  assert.equal(balance(lock.destination),0n);
...
for(const lock of locks){const ix=await proposal(lock);advance(86401);send([ix]);assert.equal(balance(lock.destination),lock.amount/36n);evidence.cases.push({category:lock.allocation.category,firstTrancheClaimed:true});}
const end=locks.reduce((m,l)=>l.start>m?l.start:m,0n)+(731n+1096n)*86400n;
advance(end-now());
for(const lock of locks){const ix=await proposal(lock);advance(86401);send([ix]);assert.equal(balance(lock.destination),lock.amount);const again=await proposal(lock);advance(86401);send([again],[owner],/NothingToClaim/);evidence.cases.push({category:lock.allocation.category,fullyReleased:true,doubleClaimRejected:true});}
