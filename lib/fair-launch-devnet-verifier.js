import { deriveProtectionAddresses } from "./solana-devnet-token.js";
...
export async function decodeLaunchPolicy(account, expectedAddress = "") {
  const bytes = decodeAccountData(account?.data);
  if (bytes.length < 158) throw new Error("The protection policy account is incomplete.");
  ...
  return {
    ...
    eligibilityRootFrozen: bytes[154] === 1,
    bump: bytes[155],
    version: readU16(bytes, 156)
  };
}
