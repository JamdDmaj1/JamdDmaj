...
export function vestedLockedAmount({ lockedAmount, cliffEndAt, releaseEndAt, timestamp }) {
  ...
  const tranche = BigInt(now - cliff) * BigInt(JAMDDMAJ_ONCHAIN_RULES.releaseTranches) / BigInt(end - cliff);
  return locked * tranche / BigInt(JAMDDMAJ_ONCHAIN_RULES.releaseTranches);
}
