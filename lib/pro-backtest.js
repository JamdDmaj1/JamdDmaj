...
function backtestAsset(asset, rows) {
  ...
  for (let index = 220; index < candles.length - 13; index += 4) {
    ...
    trades.push({
      ...
      candleDecision: setup.candleDecision,
      candlePatterns: setup.candlePatterns,
  ...
  const aiFiltered = candleFiltered.filter((trade) => Number(trade.volumeRatio || 0) >= 1.2 && Number(trade.adx || 0) >= 22);
  const aiWins = aiFiltered.filter((trade) => trade.outcome === "WIN").length;
  const aiPnl = aiFiltered.map((trade) => trade.estimatedRoe);
  const chronologicalValidation = buildChronologicalValidation(decided);
  return {
    symbol: asset.symbol,
    pair: asset.pair,
    ...
    aiRecommendationStudy: {
      mode: "Simulation-only confirmation study: aligned 4h setup, no strong opposing candle, volume at least 1.20x, and ADX at least 22.",
      decided: aiFiltered.length,
      rejected: decided.length - aiFiltered.length,
      wins: aiWins,
      losses: aiFiltered.length - aiWins,
      winRate: aiFiltered.length ? round((aiWins / aiFiltered.length) * 100) : 0,
      expectancyRoe: aiPnl.length ? round(aiPnl.reduce((sum, value) => sum + value, 0) / aiPnl.length) : 0,
      maxDrawdownRoe: calculateDrawdown(aiPnl)
    },
    chronologicalValidation
  };
}

export function buildChronologicalValidation(trades = [], trainingFraction = 0.7) {
  const ordered = [...trades]
    .filter((trade) => ["WIN", "INVALIDATED"].includes(trade?.outcome))
    .sort((a, b) => Number(a.time || 0) - Number(b.time || 0));
  const splitIndex = Math.max(1, Math.min(ordered.length, Math.floor(ordered.length * trainingFraction)));
  const summarize = (rows) => {
    ...
    return {
      ...
      expectancyRoe: rows.length ? round(rows.reduce((sum, trade) => sum + Number(trade.estimatedRoe || 0), 0) / rows.length) : 0
    };
  };
  const training = ordered.slice(0, splitIndex);
  const validation = ordered.slice(splitIndex);
  return {
    mode: "Chronological 70/30 split; the final 30% is held out from tuning.",
    training: summarize(training),
    validation: summarize(validation),
    sufficientValidation: validation.length >= 30
  };
}
...
function summarizeBacktest(perSymbol, startedAt) {
  ...
  const candleWeightedExpectancy = candleDecided
    ...
    : 0;
  const aiDecided = perSymbol.reduce((sum, item) => sum + Number(item.aiRecommendationStudy?.decided || 0), 0);
  const aiWins = perSymbol.reduce((sum, item) => sum + Number(item.aiRecommendationStudy?.wins || 0), 0);
  const aiWeightedExpectancy = aiDecided
    ? perSymbol.reduce((sum, item) => sum + Number(item.aiRecommendationStudy?.expectancyRoe || 0) * Number(item.aiRecommendationStudy?.decided || 0), 0) / aiDecided
    : 0;
  const validationSamples = perSymbol.reduce((sum, item) => sum + Number(item.chronologicalValidation?.validation?.samples || 0), 0);
  const validationWins = perSymbol.reduce((sum, item) => sum + Number(item.chronologicalValidation?.validation?.wins || 0), 0);
  const validationExpectancy = validationSamples
    ? perSymbol.reduce((sum, item) => sum + Number(item.chronologicalValidation?.validation?.expectancyRoe || 0) * Number(item.chronologicalValidation?.validation?.samples || 0), 0) / validationSamples
    : 0;
  return {
    ok: true,
    ...
    aiRecommendationStudy: {
      mode: "Simulation-only A/B confirmation study; it does not change executor decisions.",
      decided: aiDecided,
      rejected: decided - aiDecided,
      wins: aiWins,
      losses: aiDecided - aiWins,
      winRate: aiDecided ? round((aiWins / aiDecided) * 100) : 0,
      expectancyRoe: round(aiWeightedExpectancy),
      winRateDelta: round((aiDecided ? (aiWins / aiDecided) * 100 : 0) - (decided ? (wins / decided) * 100 : 0)),
      expectancyDeltaRoe: round(aiWeightedExpectancy - weightedExpectancy)
    },
    outOfSampleValidation: {
      mode: "Chronological final 30% held out from tuning.",
      samples: validationSamples,
      wins: validationWins,
      losses: validationSamples - validationWins,
      winRate: validationSamples ? round((validationWins / validationSamples) * 100) : 0,
      expectancyRoe: round(validationExpectancy),
      sufficientEvidence: validationSamples >= 100
    },
    perSymbol
  };
}
