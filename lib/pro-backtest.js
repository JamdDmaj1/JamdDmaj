...
export async function runProBacktest({ force = false } = {}) {
  ...
  const rows = await mapWithConcurrency(assets, 4, async (asset) => {
    try {
      const response = await fetchJson(`https://api.bitget.com/api/v2/mix/market/candles?symbol=${encodeURIComponent(asset.pair)}&productType=USDT-FUTURES&granularity=1H&limit=500`);
  ...
  await redisRequest("pipeline", [
    ...
    ["EXPIRE", BACKTEST_KEY, Math.round(CACHE_MS / 1000)]
  ]).catch(() => {});
  return result;
}
...
function backtestAsset(asset, rows) {
  ...
  for (let index = 220; index < candles.length - 13; index += 4) {
    ...
    trades.push({ ...outcome, side: setup.side, entry: setup.entry, time: candles[index].time });
    index += 8;
  }
...
async function fetchJson(url) {
  ...
  try {
    const response = await fetch(url, { signal: controller.signal, headers: { "User-Agent": "JamdDmaj-Pro-Backtest/1.30" } });
