...
const PROTECTION_TRIGGER_MOVE = 0.003;
const PROTECTION_LOCK_MOVE = 0.001;
...
export async function runProCycle({ force = false } = {}) {
  ...
  const threshold = config.quality === "A" ? 11 : 10;
  ...
  return { ...status, stats, signals: created, events: monitored.events, telegram };
}
...
function buildServerSignal(data) {
  ...
  const notOverextended = side === "LONG"
    ...
    : data.bollingerPosition >= -1.05;
  if (!checks.trend || !checks.higherTrend || !checks.execution || !checks.volume || !notOverextended || !capExecutionQuality) return null;
  ...
  return {
    ...
    summary: `${side} setup confirmed on 1h and 4h. Passed: ${passed.join(", ")}. Volume, liquidity, spread, funding, momentum and volatility were checked; the red level is a suggested invalidation, not an executed stop.`
  };
}
...
async function monitorOpenSignals(open, marketMap, universeMap) {
  ...
  for (const original of open) {
    ...
    const protectionWasActive = signal.protectionActive === true;
    for (const candle of touch.candles) {
      ...
      if (protectionWasActive && signal.protectedPrice) {
        const protectionTouched = long ? candle.low <= signal.protectedPrice : candle.high >= signal.protectedPrice;
      ...
      if (!signal.protectionActive && favorableMove >= PROTECTION_TRIGGER_MOVE) {
        signal.protectionActive = true;
        signal.protectedPrice = signal.entry * (1 + direction * PROTECTION_LOCK_MOVE);
...
function calculateStats(open, history) {
  const successful = history.filter((signal) => (
    signal.reached?.includes("TP1")
    || ["WIN", "PROTECTED_WIN", "PARTIAL_WIN"].includes(signal.status)
  )).length;
  const invalidated = history.filter((signal) => signal.status === "REVERSAL" && !signal.reached?.includes("TP1")).length;
  ...
  const decided = successful + invalidated;
  return {
    open: open.length,
    ...
    stale,
    tp1: successful,
    winRate: decided ? Number(((successful / decided) * 100).toFixed(1)) : 0
  };
}
...
async function sendCycleNotifications(signals, events) {
  ...
  if (!token || !chatId) {
    return { configured: false, attempted: 0, sent: 0, errors: 0 };
  }
  let attempted = 0;
  let sent = 0;
  let errors = 0;
  ...
  return { configured: true, attempted, sent, errors };
}
...
function formatSignalMessage(signal) {
  ...
  return [
    ...
    "",
    "\u{26A0}\u{FE0F} Educational monitoring only. No automatic orders or guaranteed results."
  ].filter(Boolean).join("\n");
}
...
function formatEventMessage(event) {
  ...
  return [
    ...
    event.type === "SECURE" || event.type === "PROFIT_LOCKED"
      ? `Suggested protected level: <code>${formatPrice(signal.protectedPrice)}</code> (about +1% ROE at 10x before fees)`
      : "",
...
async function fetchJson(url) {
  ...
  try {
    const response = await fetch(url, {
      ...
      headers: { "User-Agent": "JamdDmaj-Pro-Scanner/1.28" }
    });
