...
const CORE_SYMBOLS = new Set(["BTC", "ETH", "SOL", "XRP"]);
...
export async function runProCycle({ force = false } = {}) {
  ...
  const scanAssets = chooseScanAssets(state.open, force);
  ...
  const monitored = await monitorOpenSignals(state.open, marketMap);
  let open = monitored.open;
  let history = mergeHistory(state.history, monitored.updated);

  const threshold = config.quality === "A" ? 8 : 6;
  ...
  for (const signal of candidates) {
    if (created.length >= config.maxSignalsPerRun) break;
    if (open.some((item) => item.pair === signal.pair && item.status === "OPEN")) continue;
    if (recentDuplicate(history, signal, config.cooldownMinutes)) continue;
    open.push(signal);
...
function chooseScanAssets(open, force) {
  const core = PRO_ASSETS.filter((asset) => CORE_SYMBOLS.has(asset.symbol));
  const rotating = PRO_ASSETS.filter((asset) => !CORE_SYMBOLS.has(asset.symbol));
  const slot = Math.floor(Date.now() / 300000) % Math.ceil(rotating.length / SCAN_BATCH_SIZE);
  const batch = force
    ? rotating.slice(slot * SCAN_BATCH_SIZE, (slot + 2) * SCAN_BATCH_SIZE)
    : rotating.slice(slot * SCAN_BATCH_SIZE, (slot + 1) * SCAN_BATCH_SIZE);
  const openSymbols = new Set(open.map((signal) => signal.baseSymbol));
  const active = PRO_ASSETS.filter((asset) => openSymbols.has(asset.symbol));
  return uniqueAssets([...core, ...batch, ...active]);
}
...
async function fetchMarketIndicators(asset) {
  const paths = [
    `https://api.binance.com/api/v3/klines?symbol=${asset.pair}&interval=1h&limit=120`,
    `https://api.binance.us/api/v3/klines?symbol=${asset.pair}&interval=1h&limit=120`,
    `https://fapi.binance.com/fapi/v1/klines?symbol=${asset.pair}&interval=1h&limit=120`
  ];
  const rows = await Promise.any(paths.map(async (url) => {
    const value = await fetchJson(url);
    if (!Array.isArray(value) || value.length < 60) throw new Error("Market unavailable");
    return value;
  }));
  ...
  const change24h = ((price - closes.at(-25)) / closes.at(-25)) * 100;
...
function buildServerSignal(data) {
  ...
  const bullish = {
    ...
    rsi: data.rsi >= 45 && data.rsi <= 68,
    volume: data.volumeRatio >= 0.95,
    momentum: data.momentum6h > 0,
    structure: data.price >= data.recentHigh * 0.985,
    bollinger: data.bollingerPosition > -0.35,
  ...
  const scoreSide = (checks) => Object.entries(checks).reduce((score, [name, pass]) => (
    score + (pass ? (name === "trend" ? 2 : 1) : 0)
  ), 0);
  const longScore = Math.min(10, scoreSide(bullish));
  const shortScore = Math.min(10, scoreSide(bearish));
  const side = longScore >= shortScore ? "LONG" : "SHORT";
  const score = Math.max(longScore, shortScore);
  const direction = side === "LONG" ? 1 : -1;
  const atrDistance = clamp(data.atrPercent * 1.35, 0.006, 0.04);
  ...
  return {
    ...
    score,
    maxScore: 10,
    confidence: score >= 8 ? "A" : score >= 6 ? "B" : "C",
    entry,
    ...
    summary: `${side} alignment passed: ${passed.join(", ")}. Entry and invalidation use ATR-based volatility, not a fixed percentage.`
  };
}

async function monitorOpenSignals(open, marketMap) {
  const updated = [];
  const events = [];
  for (const original of open) {
    const signal = { ...original, reached: [...(original.reached || [])] };
    if (signal.status !== "OPEN") continue;
    const asset = PRO_ASSETS.find((item) => item.symbol === signal.baseSymbol);
    if (!asset) continue;
    let touch;
    try {
...
async function fetchTouchRange(asset, createdAt, validUntil) {
  ...
  const url = `https://api.binance.com/api/v3/klines?symbol=${asset.pair}&interval=5m&startTime=${start}&limit=30`;
  const rows = await fetchJson(url);
  if (!Array.isArray(rows) || !rows.length) throw new Error("Touch data unavailable");
...
async function fetchJson(url) {
  ...
  try {
    const response = await fetch(url, {
      ...
      headers: { "User-Agent": "JamdDmaj-Pro-Scanner/1.25" }
    });
...
function nextBatchLabel() {
  const rotating = PRO_ASSETS.filter((asset) => !CORE_SYMBOLS.has(asset.symbol));
  const slot = (Math.floor(Date.now() / 300000) + 1) % Math.ceil(rotating.length / SCAN_BATCH_SIZE);
  return rotating.slice(slot * SCAN_BATCH_SIZE, (slot + 1) * SCAN_BATCH_SIZE).map((asset) => asset.symbol).join(", ");
}
