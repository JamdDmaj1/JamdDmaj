...
export const MIN_LIQUIDITY_LOCK_SECONDS = 731n * 86_400n;
...
export async function createMockLiquidityLockOnDevnet({ policyAddress, wallet, account }) {
  if (!wallet || !account) throw new Error("Conecta una wallet Solana.");
  const signFeature = wallet.features?.["solana:signTransaction"];
  if (!signFeature?.signTransaction) {
    throw new Error("La prueba de liquidez requiere una wallet que permita revisar y firmar la transaccion.");
  }
  const versions = signFeature.supportedTransactionVersions || [];
  if (!versions.includes(0)) throw new Error("La wallet no admite transacciones Solana v0.");

  const rpc = createSolanaRpc(DEVNET_RPC_URL);
  await assertProtectionProgramAvailable(rpc);
  const ownerAddress = address(account.address);
  const canonicalPolicyAddress = address(policyAddress);
  const payer = createNoopSigner(ownerAddress);
  const lpMint = await generateKeyPairSigner(false);
  const mintSpace = 82n;
  const rent = await rpc.getMinimumBalanceForRentExemption(mintSpace).send();
  const latest = await rpc.getLatestBlockhash({ commitment: "confirmed" }).send();
  const sourceInstruction = await getCreateAssociatedTokenInstructionAsync({
    payer,
    owner: ownerAddress,
    mint: lpMint.address,
    tokenProgram: TOKEN_2022_PROGRAM_ADDRESS
  });
  const sourceAddress = sourceInstruction.accounts[1].address;
  const protection = await deriveLiquidityLockAddresses(canonicalPolicyAddress, lpMint.address, ownerAddress);
  const instructions = [
    getCreateAccountInstruction({
      payer,
      newAccount: lpMint,
    ...
    await getInitializeLiquidityLockInstruction({
      policyAddress: canonicalPolicyAddress,
      sourceOwnerAddress: ownerAddress,
      beneficiaryAddress: ownerAddress,
      lpMintAddress: lpMint.address,
      liquidityLockAddress: protection.liquidityLockAddress,
      liquidityVaultAddress: protection.liquidityVaultAddress,
      sourceAddress,
      amount: 1n
    }),
    getSetAuthorityInstruction({
      owned: lpMint.address,
      owner: ownerAddress,
      authorityType: AuthorityType.MintTokens,
      newAuthority: null
    })
  ...
  const compiled = compileTransaction(message);
  const mintSignedForSimulation = await partiallySignTransactionWithSigners([lpMint], compiled);
  const simulation = await rpc.simulateTransaction(
    getBase64Decoder().decode(getTransactionEncoder().encode(mintSignedForSimulation)),
    { encoding: "base64", sigVerify: false, commitment: "confirmed" }
  ).send();
  if (simulation.value.err) {
    throw new Error(`Devnet liquidity simulation failed: ${JSON.stringify(simulation.value.err, (_, value) => typeof value === "bigint" ? value.toString() : value)}`);
  }

  const [walletResult] = await signFeature.signTransaction({
    account,
    chain: DEVNET_CHAIN,
    transaction: getTransactionEncoder().encode(compiled),
    options: { preflightCommitment: "confirmed" }
  });
  if (!walletResult?.signedTransaction) throw new Error("La wallet no devolvio la transaccion firmada.");
  const walletSigned = getTransactionDecoder().decode(walletResult.signedTransaction);
  const fullySigned = await partiallySignTransactionWithSigners([lpMint], walletSigned);
  const signature = await rpc.sendTransaction(
    getBase64Decoder().decode(getTransactionEncoder().encode(fullySigned)),
    { encoding: "base64", skipPreflight: false, preflightCommitment: "confirmed", maxRetries: 3 }
  ).send();
  return Object.freeze({
    chain: DEVNET_CHAIN,
    mock: true,
    policyAddress: String(canonicalPolicyAddress),
    lpMintAddress: String(lpMint.address),
    sourceAddress: String(sourceAddress),
    liquidityLockAddress: String(protection.liquidityLockAddress),
    liquidityVaultAddress: String(protection.liquidityVaultAddress),
    lockedAmount: "1",
    lockSeconds: MIN_LIQUIDITY_LOCK_SECONDS.toString(),
    mintAuthorityRevoked: true,
    signature
  });
}

export async function deriveLiquidityLockAddresses(policyAddress, lpMintAddress, beneficiaryAddress) {
  const programAddress = address(JAMDDMAJ_LOCK_PROGRAM_ADDRESS);
  const addressEncoder = getAddressEncoder();
  const liquidityLockAddress = (await getProgramDerivedAddress({
    programAddress,
    seeds: [
      new TextEncoder().encode("liquidity"),
      addressEncoder.encode(address(policyAddress)),
      addressEncoder.encode(address(lpMintAddress)),
      addressEncoder.encode(address(beneficiaryAddress))
    ]
  }))[0];
  const liquidityVaultAddress = (await getProgramDerivedAddress({
    programAddress,
    seeds: [new TextEncoder().encode("liquidity-vault"), addressEncoder.encode(liquidityLockAddress)]
  }))[0];
  return Object.freeze({ liquidityLockAddress, liquidityVaultAddress });
}
...
export async function getInitializeCreatorVestingInstruction({
  ...
  return {
    ...
    accounts: [
      { address: policyAddress, role: AccountRole.WRITABLE },
      { address: ownerAddress, role: AccountRole.WRITABLE_SIGNER },
      { address: address(beneficiaryAddress), role: AccountRole.READONLY },
      { address: mintAddress, role: AccountRole.READONLY },
      { address: mintAddress, role: AccountRole.READONLY },
...
export async function getInitializeLiquidityLockInstruction({
  policyAddress,
  sourceOwnerAddress,
  beneficiaryAddress,
  lpMintAddress,
  ...
  sourceAddress,
  amount,
  requestedLockSeconds = MIN_LIQUIDITY_LOCK_SECONDS
}) {
  const lockAmount = BigInt(amount);
  const lockSeconds = BigInt(requestedLockSeconds);
  if (lockAmount <= 0n || lockAmount > MAX_U64) throw new RangeError("Liquidity receipt amount must be positive.");
  if (lockSeconds < MIN_LIQUIDITY_LOCK_SECONDS) throw new RangeError("Liquidity receipts must remain locked for at least 24 months.");
  return {
    programAddress: address(JAMDDMAJ_LOCK_PROGRAM_ADDRESS),
    accounts: [
      { address: address(policyAddress), role: AccountRole.READONLY },
      { address: address(sourceOwnerAddress), role: AccountRole.WRITABLE_SIGNER },
      { address: address(beneficiaryAddress), role: AccountRole.READONLY },
      { address: address(lpMintAddress), role: AccountRole.READONLY },
      { address: address(liquidityLockAddress), role: AccountRole.WRITABLE },
      { address: address(liquidityVaultAddress), role: AccountRole.WRITABLE },
      { address: address(sourceAddress), role: AccountRole.WRITABLE },
      { address: TOKEN_2022_PROGRAM_ADDRESS, role: AccountRole.READONLY },
      { address: SYSTEM_PROGRAM_ADDRESS, role: AccountRole.READONLY }
    ],
    data: concatBytes(
      await anchorDiscriminator("initialize_liquidity_lock"),
      encodeU64(lockAmount),
      encodeI64(lockSeconds)
    )
  };
}

export async function getReleaseLiquidityInstruction({
  policyAddress,
  beneficiaryAddress,
  lpMintAddress,
  liquidityLockAddress,
  ...
  return {
    ...
    accounts: [
      { address: address(policyAddress), role: AccountRole.READONLY },
      { address: address(beneficiaryAddress), role: AccountRole.WRITABLE_SIGNER },
      { address: address(lpMintAddress), role: AccountRole.READONLY },
      { address: address(liquidityLockAddress), role: AccountRole.WRITABLE },
      { address: address(liquidityVaultAddress), role: AccountRole.WRITABLE },
    ...
    data: await anchorDiscriminator("release_liquidity")
  };
}

export async function getClaimVestedInstruction({
  policyAddress,
  beneficiaryAddress,
  mintAddress,
  vestingAddress,
  ...
  return {
    ...
    accounts: [
      { address: address(policyAddress), role: AccountRole.READONLY },
      { address: address(beneficiaryAddress), role: AccountRole.WRITABLE_SIGNER },
      { address: address(vestingAddress), role: AccountRole.WRITABLE },
      { address: address(mintAddress), role: AccountRole.READONLY },
      { address: address(vaultAddress), role: AccountRole.WRITABLE },
      { address: address(destinationAddress), role: AccountRole.WRITABLE },
      { address: TOKEN_2022_PROGRAM_ADDRESS, role: AccountRole.READONLY }
    ],
    data: await anchorDiscriminator("claim_vested")
  };
}
