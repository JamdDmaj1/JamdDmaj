...
import {createTestFileJournal} from './lib/test-transfer-journal.mjs';
...
try{
 ...
 const faucet=await devnetRpc('requestAirdrop',[original,10000000,{commitment:'confirmed'}]);
 let funded=false;
 ...
 const journalPath=join(mkdtempSync(join(tmpdir(),'jamddmaj-devnet-journal-')),'transfer.json');
 console.log('Public transaction-status journal: '+journalPath);
 const session=recovered.transferSession({journal:createTestFileJournal(journalPath)});const review=await session.prepare({recipient,amountSOL:'0.001'});
...
}catch(error){console.error(JSON.stringify({testOnly:true,result:'incomplete',reason:error.message,rpcCode:error.rpcCode??null}));process.exitCode=1;}
