...
Fault-suite result (2026-09-24): Actions run `36064620975`, code `b6bcf2050ef5bcc6887b6d7645bd26969c2379b8`, passed compilation of debug/release/preview, manifest checks, 227 JavaScript tests and all 11 native Robolectric tests (zero errors, failures or skipped tests). Added readback after AtomicFile writes for the vault and metadata/journal, because a failed rename may be logged without throwing. The obstruction test verifies that an unreadable/uncommitted journal prevents broadcast. Robolectric does not validate real hardware or power-loss durability. Preview signing keys are ephemeral CI debug keys: do not promise these artifacts can update an existing preview installation; do not uninstall it without verified recovery and explicit user direction.
