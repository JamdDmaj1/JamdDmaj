...
test("asset research UI is internal, mobile-safe and translated in every supported language", async () => {
  ...
  const dexLiteral = html.match(/const DEX_METRIC_TEXT = (\{[\s\S]*?\});\s*const CHART_INTERACTION_TEXT/)?.[1];
  ...
  const chartLiteral = html.match(/const CHART_INTERACTION_TEXT = (\{[\s\S]*?\});\s*const UI_EXTENDED/)?.[1];
  assert.ok(chartLiteral, "chart interaction locale catalog should be present");
  const chartCatalog = Function(`"use strict"; return (${chartLiteral});`)();
  const chartKeys = Object.keys(chartCatalog.en).sort();
  languages.forEach((language) => assert.deepEqual(Object.keys(chartCatalog[language]).sort(), chartKeys));
  assert.match(html, /data-indicator="sma20"/);
  assert.match(html, /data-indicator="volume"/);
